...
class FileCleaner {
  constructor() {
    ...
    this.maliciousPatterns = {
      ...
      // Executable signatures (PE headers)
      peHeaders: [
        /MZ[\x00-\xFF]{58}PE\x00\x00/g,
      ],
  ...
  async cleanFile(filePath) {
    try {
      ...
      const unsafeExtensions = ['.exe', '.dll', '.sys', '.bat', '.cmd', '.com', '.scr', '.vbs', '.jar'];
...
export default new FileCleaner();
